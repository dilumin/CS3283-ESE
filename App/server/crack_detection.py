...
def crack_detection(img):
    ...
    # API endpoint
    url = "http://172.20.10.3/get-distance"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Parse the distance value using regex
            match = re.search(r"Distance \(cm\): ([\d.]+)", raw_response)
            if match:
                distance = float(match.group(1))  # Extract and convert to float
                print(f"Distance fetched from server: {distance} cm")
            else:
                print("Distance value not found in response body.")
        else:
    ...
    perpendicular_distances = [round(d*0.06*distance , 4) for d in perpendicular_distances]
